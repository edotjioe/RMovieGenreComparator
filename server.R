...
function(input, output, session) {
  ...
  genre.ratings.yearly <- reactive({
    genre.movies %>%
      filter(dataset == input$datasetp3 & genre %in% input$genrep3) %>%
  ...
  output$plot1 <- renderPlotly({
    ggplot(genre.ratings(), aes(genre, y, fill = dataset), position = "group", xlab = input$xcolp1) +
      geom_col(position="dodge", stat="identity", width = 0.8) +
  ...
  output$datatable1 = DT::renderDataTable({
    ...
    datatable(table1, filter = 'top', options = list(
  ...
  output$datatable2 = DT::renderDataTable({
    datatable(genre.movies, filter = 'top', options = list(
      pageLength = 10, autoWidth = TRUE
    ))
  })
  ...
  observeEvent(
    {
      ...
      output$plot2 <- renderPlotly(
          plot_ly(labels = pie$genre, values = pie$y, type = 'pie')
      )
  })
  
  #Rendering Timelapse chart
  output$plot3 <- renderPlotly({
    p <- ggplot(data = genre.ratings.yearly(), aes(x = year, y = avg_rating, group = genre, color = genre, size = 1)) +
      geom_line(alpha = 0.5) +
  ...
  observeEvent({
    ...
    output$p1min <- renderInfoBox({
      infoBox(
        result.min$genre, paste(result.min[, input$xcolp1]), icon = icon("chevron-down"),
        color = "red"
      )
    })
    ...
    output$p1max <- renderInfoBox({
      infoBox(
        result.max$genre, paste(result.max[, input$xcolp1]), icon = icon("chevron-up"),
        color = "green"
      )
    })
    ...
    output$p1mean <- renderInfoBox({
      infoBox(
        "Mean", round(colMeans(info[, input$xcolp1]), 2), icon = icon("minus"),
        color = "blue"
      )
    })
  })
