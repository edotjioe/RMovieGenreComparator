...
con <- dbConnect(RMySQL::MySQL(), dbname = "imdb", password = "root", user = "root")
...
movielens <- read.csv(source,header=TRUE, sep=",", row.names=NULL, stringsAsFactors = FALSE)
...
source <- paste(downloadpath,fname,sep="")
movielens.ratings <- read.csv(source,header=TRUE, sep=",", row.names=NULL, stringsAsFactors = FALSE)
...
genre.movies <- data.frame(title = character(), year = numeric(), rating = numeric(), votes = numeric(), genre = character(), dataset = character(), stringsAsFactors = FALSE)
...
genre.amount <- do.call(rbind, lapply(genre.amount, data.frame, stringsAsFactors=FALSE))
colnames(genre.amount) <- "genre_count"

#Changing list to data frame
library(plyr)
temp.genres <- ldply(temp.genres, function(x) data.frame(x, stringsAsFactors = FALSE))
...
tmdb.shrinked <- tmdb[, c("id", "title", "release_date", "vote_average", "vote_count")]
tmdb.shrinked <- cbind(tmdb.shrinked, genre.amount)
tmdb.shrinked <- transform(tmdb.shrinked, year = data.frame(year = sapply(tmdb.shrinked$release_date, 
                                                                          function(x) stri_sub(str = x,from = 1, to = 4) ), 
                                                            stringsAsFactors = FALSE))
tmdb.shrinked <- tmdb.shrinked[, c(1,2,7,4,5,3, 6)]

#Adding the changed data to tmdb.expanded data frame
tmdb.expanded <- tmdb.shrinked[rep(row.names(tmdb.shrinked), tmdb.shrinked$genre_count), 2:5]
...
movielens.ratings.temp <- aggregate(x = movielens.ratings$rating, 
                                    FUN = mean, 
                                    by = list(movielens.ratings$movieId))
names(movielens.ratings.temp)[names(movielens.ratings.temp)=="Group.1"] <- "movieId"
names(movielens.ratings.temp)[names(movielens.ratings.temp)=="x"] <- "rating"

#Adding the new columns to movielens.ratings: votes 
movielens.ratings <- cbind(movielens.ratings.temp, votes = aggregate(x = movielens.ratings$count, 
                                                                     FUN = sum, 
                                                                     by = list(movielens.ratings$movieId))$x)

#Joining dataframes
movielens <- inner_join(movielens, movielens.ratings, by = "movieId")

#Adding year as column to movielens data frame
year <- data.frame(year = sapply(movielens$title, function(x) stri_sub(str = x,from = -5, to = -2) ))
movielens <- cbind(movielens, year = year)

#Creating multiple instances of each movie by the amount of genres
movielens <- transform(movielens, genre_count = sapply(strsplit(movielens$genres, "\\|"), length))
movielens.expanded <- movielens[rep(row.names(movielens), movielens$genre_count), 2:6]

#Creating one big list with all genres in order
temp.genres <- unlist(strsplit(movielens$genres, "\\|"))
#Adding the genres to the movielens.expanded list
movielens.expanded <- transform(movielens.expanded, genres = temp.genres)
